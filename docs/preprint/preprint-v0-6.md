...
**Draft v0.6 — 2026-08-26 — not for circulation**
...
***L*_*U*(*M*₀) is now defined, and the recomputation has been done.** The framework fixes, for every model *T* of the family including *M*₀ and with no special treatment,

  ***L*_*U*(*T*) = *L*_*U*(id(*T*) | *M*) + *L*_*U*(spec(*T*) | *U*_ref) + *L*_*U*(*θ*_*T*)**,

where the identifier is a prefix-free code declared in advance — uniform, ⌈log₂ *k*⌉ bits for *k* first-level alternatives — and spec(*T*) = 0 **if and only if** *T* is a declared primitive of the reference stack. *M*₀ pays its identifier like everyone else: **the null model is not free**. For the pilot the first-level family is enumerated as {*M*₀ literal, zlib, bz2, lzma, generative}, so *k* = 5, the identifier is 3 bits for every member, the literal code and the three compressors are declared primitives, and

  ***L*_*U*(*M*₀) = 3 + 0 + 0 = 3 bits*,  *L*₀^full = 3 + *L*₀**.

This value is **not fitted to the results, and it is not a canonical property of null models**: ⌈log₂ 5⌉ = 3 is exact arithmetic, but *which* five members constitute the first-level family and *that* the identifier code is uniform and prefix-free are **conventions of the reference stack**, declared in advance. Under a different declared family or a non-uniform prefix code, *L*_*U*(*M*₀) would take a different value, and the accounting would have to be redone under that declaration. What the framework fixes is the rule; what a domain fixes is its instantiation.

The full definition, including the code tree for hierarchical families, is `protocol/definicion-L-M0.md`, incorporated by reference into §7 of the consolidated protocol. Earlier reports **stated** a log₂ 3 ≈ 1.585-bit selection charge for the generic-compressor meta-class, but the traced numerical totals **did not include** it: the historical Lorenz total is 1096 + 192 + 1248 = 2536 bits, which contains no selection term at all, against 3 + 1096 + 192 + 1248 = 2539 bits under the present accounting. The unified 3-bit first-level identifier therefore **replaces a stated convention that was never carried into the numbers**, and it *is* explicitly included in every recomputed full-accounting figure. It is not a re-labelling of 1.585 bits that had already been paid. This discrepancy is recorded as Erratum 7 in `docs/ERRATA-piloto-v1-1.md`.

**Effect on the numbers of §6, measured rather than assumed.** `src/contabilidad_completa.py` recomputes all seven rows with the complete baseline and with every model paying its identifier (`results/contabilidad-completa-salida.txt`). The pre-declared prediction was that the shift would fall below the rounding of the tables. **It did, in all seven rows:** the largest |*ρ*_full − *ρ*_bare| is 2.9 × 10⁻⁵ (Rules 110 and 30), and **no row changes at four decimals**. *ρ*_full ≤ 1 holds in all seven, exactly and by construction, with *ρ*_full = 1.000000 for the noise control, where *M*₀ wins. The tables in §6 therefore stand as printed; what has changed is that they are now normalised by a baseline that is defined.
...
Both ratios require *L*_*U*(*M*₀), which is defined in `protocol/definicion-L-M0.md` and summarised in §4.3; for the pilot it equals 3 bits, the identifier of the null model within a first-level family of five. *L*_*U*(*T*_train) is built the same way: identifier plus specification plus parameters.
...
which charges the model in the numerator without crediting *M*₀ in the denominator. Every historical *g* value in the pilot literature — 0.0088, 1.009, 1.0088 — is a *g*_total^bare. It is reported under that name in §6.2 beside the symmetric *g*_total, and is **not** silently reinterpreted as it. Historical cases (the positron; the Higgs mechanism, but not the Higgs mass, which the Standard Model left free) are approximations to this criterion, not literal runs of it.
...
under explicit conditions, given below. Writing *A* = *L*_*U*(*T*_train), *B* = *L*_*U*(*M*₀), *G* = *L*_*U*(*D*_test, *ε* | *T*_train) and *H* = *L*_*U*(*D*_test, *ε* | *M*₀), so that *g*_pred = *G*/*H* and *g*_total = (*A*+*G*)/(*B*+*H*), the exact identity is

  ***g*_total − *g*_pred = (*A* − *B*·*g*_pred) / (*B* + *H*) = (*AH* − *BG*) / [*H*(*B* + *H*)]**.

Three things follow, and none of them is an inequality between the two ratios. **First**, the *sign* of the difference is the sign of *A* − *B*·*g*_pred: it is positive when the trained model's description exceeds the null model's description scaled by the predictive ratio, negative otherwise, and there is no universal direction. **Second**, the difference reduces to *A*/(*B* + *H*) **only in the special case *g*_pred = 0**; using that expression when *g*_pred > 0 overstates the difference by *B*·*g*_pred/(*B* + *H*). **Third**, if *A* and *B* are fixed and *g*_pred stays bounded, then as the test block grows *H* → ∞ and the difference tends to zero, because the fixed description costs are amortised over more data. We do **not** claim the limit is approached from above, or from any particular side.

A *g*_total whose distance from *g*_pred does not shrink as the test block grows indicates a model whose description grows with the data it explains — the signature of fitting rather than compressing, and a result to be reported as such. Beyond these, robustness to the preregistered decisions is required; and, in addition, the advantage must (1) persist as *n* grows; (2) persist as resolution increases within the studied regime; (3) survive on *D*_test; (4) fall significantly on surrogates; (5) survive across reference stacks; (6) not be explicable by theoretical contamination of the pipeline; (7) be compared across domains only at comparable descriptive levels.
...
The *ρ* values are those printed by the pilot's scripts, normalised as they were; recomputing all seven rows with the complete baseline *L*₀^full and with every model paying its 3-bit identifier changes no entry at four decimals (largest shift 2.9 × 10⁻⁵; §4.3), and gives *ρ*_full ≤ 1 in all seven, with exactly 1.000000 for the noise row.
...
|---|---|---|---|---|
| Rule 110 | 0.0000 | **0.0000** | 0.0088 | **0.0088** |
| Rule 30 | 0.0000 | **0.0000** | 0.0088 | **0.0088** |
| Noise, PCG64 seed 2026 | 0.5001 | **1.0000** | 1.0088 | **1.0088** |
| Noise, `os.urandom` (unseeded) | ≈ 0.50 | ≈ 1 | 1.009 | invariants only |

with *A* = *L*_*U*(*T*_train) = 811 bits (3 identifier + 800 compressed source + 8 rule), *B* = *L*_*U*(*M*₀) = 3 bits and *H* = *L*_*U*(*D*_test, *ε* | *M*₀) = 92 160 bits, so that the symmetric denominator *B* + *H* = 92 163 bits.

**The difference *g*_total − *g*_pred is not the same number in the three controls**, although all three round to 0.0088 at four decimals. By the identity of §4.5:

| Control | *g*_pred | *g*_total − *g*_pred | value | rounds to |
|---|---|---|---|---|
| Rule 110 | 0 exactly | *A*/(*B*+*H*) = 811/92 163 | 0.0087996 | 0.0088 |
| Rule 30 | 0 exactly | *A*/(*B*+*H*) = 811/92 163 | 0.0087996 | 0.0088 |
| Noise, PCG64 seed 2026 | 0.999999978 | (*A* − *B*·*g*_pred)/(*B*+*H*) ≈ (*A*−*B*)/(*B*+*H*) = 808/92 163 | 0.0087671 | 0.0088 |

The reduction to *A*/(*B*+*H*) holds for the two cellular automata **because their predictive residual is exactly zero**, not as a general rule; for the noise control, where *g*_pred ≈ 1, the correct numerator is *A* − *B*·*g*_pred ≈ 808 bits, not 811. `src/contabilidad_completa_v2.py` prints both sides of the identity without internal rounding and verifies their agreement by assertion (`results/contabilidad-completa-v2-salida.txt`). No published figure changes: the four-decimal table above stands as printed.

**Which quantity these numbers are.** The historical pilot values 0.0088, 1.009 and 1.0088 are ***g*_total^bare**, not *g*_total: `piloto_10_1.py` computes `(L(T) + L_test)/tot`, charging the model in the numerator without crediting *M*₀ in the denominator. They are **not reinterpreted** as the symmetric *g*_total of §4.5. The *g*_pred column is printed by `src/g_metricas_oos.py` (`results/g-metricas-oos-salida.txt`), a script added for this draft that recomputes the out-of-sample procedure with the pilot's own seeds and prints *g*_pred and *g*_total^bare side by side; it reproduces the historical *g*_total^bare exactly and modifies no earlier script or output. The symmetric *g*_total is now computed by `src/contabilidad_completa.py` (`results/contabilidad-completa-salida.txt`) under the definition of *L*_*U*(*M*₀) given in §4.3; no value was assumed for it, and the historical column is retained beside it rather than replaced. That the two columns agree to four decimals here is a consequence of *L*_*U*(*M*₀) = 3 bits being negligible against a 92 160-bit test block — not of the two quantities being the same. The unseeded `os.urandom` row reports only invariants, per Amendment 6(b).
...
*Traceability note (Amendment 6).* The Level A figure is **1288 bits**, not the "≈ 1400 bits" of the original pilot report, which used the stale source-length constant 1252 corrected in §7.5. The pilot report's own errata section does not yet carry this correction; it is recorded in `docs/ERRATA-piloto-v1-1.md` and listed as a pending action. Under the traceability rule, the Level A subtotal, the Level B subtotal and the total are **printed directly** by `src/contabilidad_completa.py` (`results/contabilidad-completa-salida.txt`) and are no longer sums performed by hand: at *n* = 20 000, identifier 3 + Level A 1288 + Level B 1248 = 2539 bits, with Level A at 0.5073 of the total.
...
- ***ρ*_MDL ≤ 1 was asserted, not guaranteed.** With the baseline taken as the bare residual *L*_*U*(*D*_ε | *M*₀), the bound is 1 + *L*_*U*(*M*₀)/*L*₀ > 1. §4.3 now normalises by the complete baseline *L*₀^full, under which the bound is exact. **Closed:** *L*_*U*(*M*₀) is defined (`protocol/definicion-L-M0.md`) and all seven rows have been recomputed; no entry changes at four decimals and *ρ*_full ≤ 1 holds row by row. **Debt:** the pilot's code never defines *L*_*U*(*M*₀), so every *ρ* in §6 is still against the bare-residual baseline. The shift is below the tables' rounding, but it has not been computed, and no value here has been silently adjusted. Fixing *L*_*U*(*M*₀) and re-running is a pending action.
- ***g* was defined as residual-only and reported as model-plus-residual, and the model-plus-residual form was itself asymmetric.** §4.5 now defines *g*_pred and a **symmetric** *g*_total in which the numerator's model cost is matched by *L*_*U*(*M*₀) in the denominator, and states that no inequality between the two ratios is assumed. **Closed:** *g*_pred is printed by `src/g_metricas_oos.py`, and the symmetric *g*_total by `src/contabilidad_completa.py`, both against the defined *L*_*U*(*M*₀). §6.2 reports the historical *g*_total^bare beside them under its own name; no historical value was reinterpreted.
...
A fourth item is a value, not a definition: the Level A cost of the Lorenz rule is **1288 bits**, not the ≈ 1400 of the pilot report, whose source-length constant was corrected by Amendment 6 (§7.5); it is now printed as a subtotal rather than summed by hand, and recorded in `docs/ERRATA-piloto-v1-1.md`. The pilot report's own errata section does not yet carry this correction.

**Two defects of the previous draft, corrected here.** (i) The difference *g*_total − *g*_pred was written as *A*/(*B* + *H*) and described as taking the same value in all three controls. That expression is valid **only when *g*_pred = 0**; the exact identity is (*A* − *B*·*g*_pred)/(*B* + *H*), and for the noise control, where *g*_pred ≈ 1, the difference is 0.0087671 rather than 0.0087996 (§4.5, §6.2). Both still round to 0.0088, so no published figure moves — the error was algebraic and interpretive, not numerical. (ii) The historical log₂ 3 selection charge was described as superseded by the 3-bit identifier, which implied it had been paid; the traced totals show it never entered them (§4.3, Erratum 7).

**What these closures do not buy.** The accounting is now internally complete and the recomputation confirmed the pre-declared prediction. None of that makes the pilot evidence about nature (§6.6), implements the two independent reference stacks (§7.2), or explains the slope discrepancy (§7.1), which was not investigated in this round.
