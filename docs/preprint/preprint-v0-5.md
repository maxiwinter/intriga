...
**Draft v0.5 — 2026-08-26 — not for circulation**
...
The full definition, including the code tree for hierarchical families, is `protocol/definicion-L-M0.md`, incorporated by reference into §7 of the consolidated protocol. It supersedes the earlier convention of charging log₂ 3 bits for choosing among the generic compressors, which is now subsumed in the uniform 3-bit identifier over the family of five.
...
whenever the model and null-model description costs are fixed and the test block grows, since both enter only the numerator or the denominator additively while the data terms grow without bound. We do **not** claim the limit is approached *from above*: the two ratios have different denominators, so the sign of *g*_total − *g*_pred is not fixed in general. It is positive throughout the pilot only because *L*_*U*(*T*_train) = 811 bits greatly exceeds *L*_*U*(*M*₀) = 3 bits there, which makes the difference 811/92 163 = 0.0088 in all three controls (§6.2). A *g*_total that stays flat, or whose distance from *g*_pred does not shrink as the test block grows, indicates a model whose description grows with the data it explains — the signature of fitting rather than compressing, and a result to be reported as such. Beyond these, robustness to the preregistered decisions is required; and, in addition, the advantage must (1) persist as *n* grows; (2) persist as resolution increases within the studied regime; (3) survive on *D*_test; (4) fall significantly on surrogates; (5) survive across reference stacks; (6) not be explicable by theoretical contamination of the pipeline; (7) be compared across domains only at comparable descriptive levels.
...
with *L*_*U*(*T*_train) = 811 bits (3 identifier + 800 compressed source + 8 rule), *L*_*U*(*M*₀) = 3 bits and *L*_*U*(*D*_test, *ε* | *M*₀) = 92 160 bits, so that the symmetric denominator is 92 163 bits. In these three controls *g*_total − *g*_pred = 811/92 163 = 0.0088 exactly, which is the amortised cost of the model over the test block and nothing else.
...
**What these closures do not buy.** The accounting is now internally complete and the recomputation confirmed the pre-declared prediction. None of that makes the pilot evidence about nature (§6.6), implements the two independent reference stacks (§7.2), or explains the slope discrepancy (§7.1).
