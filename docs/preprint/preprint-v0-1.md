...
> **SUPERSEDED** — este archivo contiene la **v0.2** y se conserva sin modificar como registro. La versión vigente es `preprint-v0-3.md`; los cambios están en `CHANGELOG-v0-2-to-v0-3.md`. (Nombre y contenido divergieron antes de adoptar la convención "un archivo por versión".)

**Draft v0.2 — 2026-08-26 — not for circulation**
...
Each domain has a null code *M*₀ that does not exploit the structure being sought — a literal or near-literal code — so that the procedure can legitimately conclude *no useful compression within the evaluated family*. The descriptive ratio is
  ...
  *ρ*(*d*, *ε*, *U*; *D*_*n*) = *C*(*d*, *U*; *D*_*n*, *ε*) / *L*_*U*(*D*_*n*, *ε* | *M*₀).

Because *M*₀ is always in the family, *C* ≤ *L*_*U*(*D*_ε | *M*₀) + *O*(1), and *ρ* ≈ 1 acquires the operational meaning "no useful compression was found". The limit *ε* → 0 of *ρ* is never interpreted in isolation.
...
A grammar can be built retrospectively to compress what it has already seen. The stronger criterion is that it keeps compressing new information. Before any model is trained or selected, the partition *D* = *D*_train ∪ *D*_test is frozen; model structure, hyperparameters, library, quantiser, metric and representation may not be modified after *D*_test is consulted. Then
  ...
  *G*(*T*) = *L*_*U*(*D*_test, *ε* | *T*_train),  ***g*(*T*) = *G*(*T*) / *L*_*U*(*D*_test, *ε* | *M*₀),**  Δ*G* = *L*_*U*(*D*_test, *ε* | *M*₀) − *G*(*T*).

*g* ≈ 1: no useful descriptive generalisation; *g* < 1: out-of-sample advantage; *g* ≪ 1: strong predictive compression; ***g* > 1: the model is worse than the baseline** — it pays its own cost without buying anything. Historical cases (the positron; the Higgs mechanism, but not the Higgs mass, which the Standard Model left free) are approximations to this criterion, not literal runs of it.
...
4. **Generalisation discriminates (H2).** The exact rule was recovered from 40 rows and sustained over 360 out-of-sample rows with *g* ≈ 0.009; on noise the procedure is not fooled: *g* > 1.
...
**The naive-coder artefact and its correction (Amendment 2).** A two-part coder with a single initial condition and a literal residual after divergence gives, for Lorenz, *ρ*_model = 0.057 at *n* = 1000, 0.024 at 2500, 0.48 at 5000, 0.74 at 10 000 and 0.87 at 20 000 — the generative advantage *appears* to be exhausted as the horizon grows. A coder with checkpoints — re-specification of the state every Lyapunov horizon of 2655 steps, ⌈*n*/2655⌉ checkpoints of 156 bits each — removes the artefact (*ρ* = 0.0602, 0.0241, 0.0133, 0.0080, 0.0053 at the same five *n*; 2536 bits in total at *n* = 20 000) and reveals the true cost structure: the rule is paid once (≈ 1 400 bits for the Lorenz rule at Level A); the particular trajectory pays, in addition, ≈ *λ*/ln 2 bits per unit time, irreducibly, at fixed resolution. This is the miniature stratigraphy *law → realisation → microstate* measured inside one system.
...
- **If H1 and H2 hold for this domain**, then within the valid regime (*ε* above the instrumental floor) the preregistered family will reach *ρ*_MDL clearly below 1 on the training corpus *and* *g* clearly below 1 on held-out elements, and *η*_ε will be ≪ 1 across the preregistered resolutions: additional wavelength precision will be absorbed by reusable structure rather than paid literally.
...
- **Falsifiers** for this domain, verbatim from §5.6: *ρ*_MDL converging towards 1; *g* ≈ 1 or > 1 on held-out elements; a qualitative change of the ordering between real and surrogate data when *U*_ref is changed; no difference between real data and surrogates; *η*_ε ≈ 1 across the valid regime.
...
The pilot code (`src/piloto_10_1.py`, `src/verificacion_piso_y_eta.py`, `src/lorenz_checkpoints.py`, `src/ruido_oos_semilla.py`), its archived outputs (`results/`), the pilot report with its errata, the protocol changelog and Amendment 6, and the reproduction record of 2026-08-26 are in the project repository. The frozen protocol v1.1 will be deposited with its SHA-256 hash in a public preregistration before the first empirical measurement; the hash will be cited here once it exists.
