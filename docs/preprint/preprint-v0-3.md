# A Preregistered Metrological Framework for Measuring the Scale- and Resolution-Dependent Descriptive Compressibility of Empirical Regularities

**Draft v0.3 — 2026-08-26 — not for circulation**

> **On the word "preregistered" in the title.** It describes the *design discipline* of this framework — every representation, metric, resolution, reference stack, null model, model family and train/test split is fixed before any test data are inspected — **not** a completed public deposit. At the time of writing, protocol v1.1 has **not** been frozen and **no public preregistration exists**; both are pending acts of the investigator (§5.8). No hash, registry identifier or deposit date is claimed anywhere in this draft.

Maximiliano Winter¹

¹ *Affiliation: [to be completed]*

> **Title note.** The title above is the one recommended by the originality audit (see `notas-auditoria-impacto-originalidad.md`). The working title of the milestone was "Measuring the Descriptive Compressibility of Scientific Regularities: A Pre-registered Framework"; it is retained here as a short running title. The final choice is the author's.
...
**Two ratios, never to be mixed.** The residual cost and the amortised model cost answer different questions, and a single symbol *g* has been used for both — including in an earlier draft of this paper. We report them separately:

  ***g*_pred = *G*(*T*) / *L*_*U*(*D*_test, *ε* | *M*₀)** — residual only: how well the frozen model describes unseen data.
  ***g*_total = [ *L*_*U*(*T*_train) + *G*(*T*) ] / *L*_*U*(*D*_test, *ε* | *M*₀)** — the model's own description charged once against the test block it must earn.

*g*_pred ≈ 1: no useful predictive generalisation; *g*_pred ≪ 1: strong predictive compression; *g*_pred = 0: the frozen model reproduces the test block exactly. *g*_total additionally asks whether the model earns its own description on that block: ***g*_total > 1 means the model is worse than the baseline** — it pays its own cost without buying enough. Necessarily *g*_total > *g*_pred; and a model that predicts perfectly can still show *g*_total > 1 on a test block small enough that its description is not amortised, which is a property of the block, not a failure of prediction. Both numbers are given at every occurrence in §6, and no value is reported as bare *g*. Historical cases (the positron; the Higgs mechanism, but not the Higgs mass, which the Standard Model left free) are approximations to this criterion, not literal runs of it.
...
and robustness to the preregistered decisions; and, in addition, the advantage must (1) persist as *n* grows; (2) persist as resolution increases within the studied regime; (3) survive on *D*_test; (4) fall significantly on surrogates; (5) survive across reference stacks; (6) not be explicable by theoretical contamination of the pipeline; (7) be compared across domains only at comparable descriptive levels.
...
**Out-of-sample generalisation** (rule inferred from 40 rows, frozen, tested on 360): Rule 110 → rule recovered exactly, 0.0 % test error, so ***g*_pred = 0** and ***g*_total = 0.0088**. Rule 30 → same, *g*_pred = 0, *g*_total = 0.0088. Noise treated as an automaton → a spurious "rule", ≈ 50 % test error, ***g*_pred ≈ 1** and ***g*_total = 1.009 > 1**: the model is worse than the baseline; it pays its own cost without buying anything. (Unseeded `os.urandom` control: only these invariants are reported. Seeded reproducible variant, PCG64 seed 2026, Amendment 6: test error 0.5001, *g*_total = 1.0088.)

The pilot scripts print only *g*_total — the quantity in the third column of `results/piloto-10-1-salida.txt` — because the model cost is included in their numerator. The *g*_pred values quoted here follow from the archived test error alone (0.0 % → residual 0 bits → *g*_pred = 0; ≈ 50 % → binary entropy 1 bit per cell → *g*_pred ≈ 1) and are **derived, not printed**. Under the traceability rule (§5.8) a script must print both before either is cited in a published table; that is a pending action (§7.5).
...
4. **Generalisation discriminates (H2).** The exact rule was recovered from 40 rows and sustained over 360 out-of-sample rows with *g*_pred = 0 and *g*_total ≈ 0.009; on noise the procedure is not fooled: *g*_pred ≈ 1, *g*_total > 1.
...
Both tracks are verifiable in `results/lorenz-checkpoints-salida.txt`; their sum is the *C* from which each *ρ* is formed. Reporting only the combined *ρ* = 0.0053 hides the fact that one track is flat and the other is linear in the horizon: at *n* = 1000 the rule is 89 % of the description, at *n* = 20 000 it is 51 %, and it keeps falling. This is the miniature stratigraphy *law → realisation → microstate* measured inside one system, and it is the reason the protocol forbids comparing domains across descriptive levels (§4.6).

*Traceability note (Amendment 6).* The Track A figure is **1288 bits**, not the "≈ 1400 bits" of the original pilot report, which used the stale source-length constant 1252 corrected in §7.5. The pilot report's own errata section does not yet carry this correction; it is listed as a pending action.
...
- ***ρ*_MDL ≤ 1 was asserted, not guaranteed.** With the baseline taken as the bare residual *L*_*U*(*D*_ε | *M*₀), the bound is 1 + *L*_*U*(*M*₀)/*L*₀ > 1. §4.3 now normalises by the complete baseline *L*₀^full, under which the bound is exact. **Debt:** the pilot's code never defines *L*_*U*(*M*₀), so every *ρ* in §6 is still against the bare-residual baseline. The shift is below the tables' rounding, but it has not been computed, and no value here has been silently adjusted. Fixing *L*_*U*(*M*₀) and re-running is a pending action.
- ***g* was defined as residual-only and reported as model-plus-residual.** §4.5 now separates *g*_pred from *g*_total. **Debt:** the scripts print only *g*_total; the *g*_pred values in §6.2 are derived from archived test errors and are labelled as derived. A script that prints both is a pending action.
...
### 7.8 A reading taken, not verified

The Track A / Track B separation in §6.3 reads "track" as the protocol's *descriptive level* — Level A, the reusable rule; Level B, the particular realisation — because that is the distinction the pilot's numbers actually decompose into. If the intended distinction was a different one, the numbers in that table are unaffected but their labels are not; the reading is flagged here rather than presented as settled.

### 7.9 Originality of *η*_ε not confirmed
...
## 8. Preregistered predictions and analysis for the first empirical measurement: atomic spectra (protocol §11.1)
...
### 8.2 What will be preregistered before *D*_test is seen

For the chosen spectral corpus: the representation (line list with wavelength, uncertainty, intensity, element and ionisation stage; units; ordering; treatment of blended or unresolved lines); the provenance class of each field, with the database version and access date; the distortion metric *d* (in the first instance, absolute and relative wavelength error, both preregistered); the resolution set *ε*₁ > … > *ε*_*m*, expressed relative to the *declared instrumental uncertainty* of each line, so that no *ε* below the measurement floor is interpreted (Amendment 1); the quantiser; the residual code; the precision rule for continuous parameters; two reference stacks with declared arithmetic; the null model *M*₀ (literal coding of the quantised line list); a model family whose **primitives and libraries do not supply the answer** — no Rydberg formula, no quantum-defect parametrisation and no level structure available as a primitive or as a charged library; the family must be able to *find* or *fail to find* such structure. We state the restriction at the level of primitives deliberately: any family expressive enough to be interesting contains such a regularity implicitly, as something it can construct, and no declaration makes that false. Preregistration does not buy a family innocent of the answer; it buys a family that must **pay** for it, the description length of whatever it constructs being charged in full, so that a rediscovered Rydberg relation competes on the same accounting as the literal line list — with members at increasing expressive power and their full description length charged; nested corpora by element and by ionisation stage, increasing diversity and not only line count; the train/test partition by element (and, within elements, by spectral region) frozen before any model is fitted; and the surrogates (wavelength shuffling within element, histogram-preserving permutation across elements).
...
- **If H1 and H2 hold for this domain**, then within the valid regime (*ε* above the instrumental floor) the preregistered family will reach *ρ*_MDL clearly below 1 on the training corpus *and* *g*_pred clearly below 1 on held-out elements (with *g*_total reported alongside it), and *η*_ε will be ≪ 1 across the preregistered resolutions: additional wavelength precision will be absorbed by reusable structure rather than paid literally.
- **The advantage will persist as *n* grows** by adding elements and ionisation stages, not only lines.
- **Surrogates will collapse**: wavelength-shuffled line lists will yield *ρ*_MDL near the null value, and the surrogate/real gap will be reported under both reference stacks.
- **Falsifiers** for this domain, verbatim from §5.6: *ρ*_MDL converging towards 1; *g*_pred ≈ 1, or *g*_total > 1 with no compensating fall in *g*_pred, on held-out elements; a qualitative change of the ordering between real and surrogate data when *U*_ref is changed; no difference between real data and surrogates; *η*_ε ≈ 1 across the valid regime.
- **Expected consequence of the oracle gap (§6.3, item 2).** A high *ρ*_MDL in this domain will *not* be reported as evidence that atomic spectra are incompressible. It will be reported as: the preregistered family did not find the structure, together with the family's description.
...
The pilot code (`src/piloto_10_1.py`, `src/verificacion_piso_y_eta.py`, `src/lorenz_checkpoints.py`, `src/ruido_oos_semilla.py`), its archived outputs (`results/`), the pilot report with its errata, the protocol changelog and Amendment 6, the reproduction record of 2026-08-26, and the changelog and validation report of this draft (`CHANGELOG-v0-2-to-v0-3.md`, `VALIDATION-REPORT-v0-3.md`) are in the project repository. **No preregistration record exists yet to cite.** Protocol v1.1, once frozen, is to be deposited with its SHA-256 hash in a public preregistration before the first empirical measurement; the hash will be cited here once it exists.
